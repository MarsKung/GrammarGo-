...
def get_configured_client():
    ...
    return OpenAI(base_url=ASSISTANT_API_BASE_URL, api_key=api_key)
...
def validate_grammar(sentence: str, grammar_rule: str) -> dict:
    ...
    try:
        ...
        run = client.beta.threads.runs.create_and_poll(thread_id=thread.id, assistant_id=assistant_id, timeout=30.0)
        ...
        if run.status == 'completed':
            ...
            error_message = f"執行失敗，狀態：{run.status}" + (f", 錯誤: {run.last_error.message}" if run.last_error else "")
            print(error_message)
            client.beta.threads.delete(thread_id=thread.id)
